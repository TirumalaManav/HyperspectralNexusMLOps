...
class HyperspectralAE(nn.Module):
    def __init__(self, in_channels, n_classes):
        ...
        self.adaptive_pool = nn.AdaptiveAvgPool2d((2, 2))  # Ensure the output is 2x2 before the fully connected layer

        self.fc1 = nn.Linear(1024 * 2 * 2, 1024)  # Adjusted for the new output size
        self.fc2 = nn.Linear(1024, n_classes)

    def forward(self, x):
        # Adjust the input tensor if it has an extra dimension
        # Reshape from [N, 1, C, H, W] to [N, C, H, W]
        if x.dim() == 5 and x.size(1) == 1:
            x = x.squeeze(1)  # This removes the extra "1" dimension

        # Now, proceed with the rest of your forward method
